...
bool cal_save(void)
{
    ...
    /* Quad-word (16 byte) program — STM32U3 flash word boyutu               */
    uint32_t addr = CAL_FLASH_ADDR;
    const uint8_t *src = (const uint8_t*)&rec;
    uint32_t bytes_remaining = sizeof(rec);

    /* Yuvarlat: 16'nın katı olacak şekilde tampona kopyala (kalan 0xFF)     */
    uint8_t buf[((sizeof(cal_record_t) + 15u) / 16u) * 16u];
